...
/**
 * 标签<context:component-scan>的解析已经分析完了，总结如下：
...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	public AnnotationConfigApplicationContext() {
		/**
		 * 注册spring内置5个组件(4个processor+1个factory)
		 * internalConfigurationAnnotationProcessor+internalCommonAnnotationProcessor+internalAutowiredAnnotationProcessor+internalEventListenerProcessor+internalEventListenerFactory
		 * 1.将组件组装到Set<BeanDefinitionHolder>中
		 * 2.同时将组件放入到beanDefinitionMap中，beanDefinitionMap.put(beanName, beanDefinition)
		 */
		this.reader = new AnnotatedBeanDefinitionReader(this);
		//Bean扫描器，mybatis的扫描类继承此类，在spring-mybatis中
	...
	public AnnotationConfigApplicationContext(Class<?>... annotatedClasses) {
		//通过AnnotatedBeanDefinitionReader初始化spring5个内置组件,并放入到beanDefinitionMap中
		this();
		/**	注册类：beanDefinitionRegistry.put(beanName, BeanDefinition)-->>>DefaultListableBeanFactory: beanDefinitionMap.put(beanName, beanDefinition)
		 *  1.检查各个注解(@Primary/@lazy等)，将值设置到beanDefinition(AbstractBeanDefinition)中
		 *  2.scopedProxyMode是否需要代理：NO=不创建 TARGET_CLASS=创建代理
		 *  3.将annotatedClasses设置到beanDeifinitionMap中
		 */
